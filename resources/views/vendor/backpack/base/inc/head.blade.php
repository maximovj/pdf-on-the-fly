    ...
    {{-- BackPack importará los scripts antes de cargar la página, solo los que sean necesarios --}}
    {{-- Se configura en config\backpack\base.php y head_scripts --}}
    @if (config('backpack.base.head_scripts') && count(config('backpack.base.head_scripts')))
        @foreach (config('backpack.base.head_scripts') as $name => $path)
            <!--- {{$name}} ---->
